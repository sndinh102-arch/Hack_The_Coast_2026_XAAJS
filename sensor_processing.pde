...
void drawRadar() {
  ...
  // NEW: dynamic radar arcs
  float radarDiameter = (width - width*0.0625);

  for (int d = 100; d <= maxRange; d += 100) {
    float r = map(d, 0, maxRange, 0, radarDiameter);
...
void drawObject() {
  ...
  // NEW: proper scaling  
  pixsDistance = map(iDistance, 0, maxRange, 0, (height - height*0.1666)); // covers the distance from the sensor from cm to pixels
...
void drawText() { // draws the texts on the screen
  ...
  // NEW: aligned labels
  float radarRadius = (width - width*0.0625) / 2;

  for (int d = 100; d <= maxRange; d += 100) {
    float r = map(d, 0, maxRange, 0, radarRadius);
    text((d/100) + "m", width/2 + r - 30, height-height*0.0833);
  }
